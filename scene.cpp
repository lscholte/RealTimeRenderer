...
void Scene::Init()
{
    ...
    AlphaMasks = packed_freelist<AlphaMask>(512);
    ...
    std::vector<uint32_t>* loadedMeshIDs)
{
    ...
    std::map<std::string, uint32_t> alphaMaskCache;
    ...
    for (const tinyobj::material_t& materialToAdd : materials)
    {
        ...
        {
            auto cachedTexture = alphaMaskCache.find(materialToAdd.alpha_texname);

            if (cachedTexture != end(alphaMaskCache))
            {
                newMaterial.AlphaMaskID = cachedTexture->second;
            }
            else
            {
                ...
                else
                {
                    ...
                    GLuint newAlphaMaskTO;
                    glGenTextures(1, &newAlphaMaskTO);
                    glBindTexture(GL_TEXTURE_2D, newAlphaMaskTO);
                    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, x, y, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels);
                    glGenerateMipmap(GL_TEXTURE_2D);
                    glBindTexture(GL_TEXTURE_2D, 0);

                    AlphaMask newAlphaMask;
                    newAlphaMask.AlphaMaskTO = newAlphaMaskTO;

                    uint32_t newAlphaMaskID = scene->AlphaMasks.insert(newAlphaMask);

                    alphaMaskCache.emplace(materialToAdd.alpha_texname, newAlphaMaskID);

                    newMaterial.AlphaMaskID = newAlphaMaskID;
